...
lmpred <- function (train, test, ratings) {
  l <- lm(ratings ~ ., data=train, na.action = "na.exclude"); 
  pred <- predict(l, test); 
  return(list(pred=pred))
}
...
gbmpred <- function (train, test, ratings) {
  gb <- gbm.fit(train, ratings, distribution='gaussian', shrinkage=0.08, n.trees=250, interaction.depth=10);
