...
women=trainfeatsData[trainfeatsData$GENDER=="Female",]
...
for(i in 9:89)
{
  temp=trainfeatsData[trainfeatsData[,i]==1,]
  wordratings[i]=mean(as.numeric(temp$Rating))
}

wr=cbind(colnames(trainfeatsData)[1:89],as.numeric(wordratings))
...
ageRatings1=trainfeatsData[trainfeatsData$AGE<20,]
ageRatings2=trainfeatsData[trainfeatsData$AGE>=20 & trainfeatsData$AGE<30,]
ageRatings3=trainfeatsData[trainfeatsData$AGE>=30 & trainfeatsData$AGE<40,]
ageRatings4=trainfeatsData[trainfeatsData$AGE>=40 & trainfeatsData$AGE<50,]
ageRatings5=trainfeatsData[trainfeatsData$AGE>=50 & trainfeatsData$AGE<60,]
ageRatings6=trainfeatsData[trainfeatsData$AGE>=60,]
...
aggdata <-aggregate(x=trainfeatsData, by=list(trainfeatsData$WORKING), 
                    FUN=mean, na.rm=TRUE)
aggdata=aggdata[-1,]
par(mar=c(5,16,1,1))
...
values=c(27.4545,25.9347,24.5728,24.7854,23.9862)
